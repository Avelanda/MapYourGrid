...
??? success "INTRODUCTION (Cliquer)"
    Bienvenue sur notre plateforme interactive dédiée à la cartographie des réseaux électriques via OpenStreetMap ! Cliquez sur un pays ou un état ci-dessous pour commencer à cartographier les infrastructures électriques directement dans JOSM. :rocket:
...
async function initQueryUI() {
  ...
  `;
  introBox.innerHTML = `
    <strong>📍TELECHARGE LE RESEAU</strong><br/>
    <ol style="margin: 0.5em 0 0 1em; padding-left: 1em;">
      <li>Clique sur le pays dont tu veux télécharger dans JOSM. Zoomer pour sélectionner une région.</li>
      <li>Clique sur Tools and Hints pour télécharger les données qui vous aidront à cartographier le réseau.</li>
      <li>N'oubliez pas de consulter le Map Legend, Good First Lines, Hot Keys and Curated Electrical Grid Maps below.</li>
      <li>N'oubliez pas qu'il s'agit d'un extrait spécifique d'OpenStreetMap et que certains autres objets existants peuvent rester masqués pendant que vous contribuez. Vérifiez toujours les données OpenStreetMap. <a href="/starter-kit/#5-load-power-infrastructure-into-josm">to avoid any conflicts or double mapping</a> on save.</li>
...
[Bonnes Premieres Lignes:fontawesome-solid-paper-plane:](good-first-lines.md){ .md-button .md-button--primary }
